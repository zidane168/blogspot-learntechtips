...
function loophalaman(banyakdata) {
  ...
  html +=
    "<span class='showpageOf'>Page " + nomerhal + " / " + maksimal + "</span>";
  ...
  if (thisUrl.indexOf("?q=") == -1 && thisUrl.indexOf(".html") == -1) {
    if (thisUrl.indexOf("/search/label/") == -1) {
      ...
      if (urlactivepage.indexOf("#PageNo=") != -1) {
        nomerhal = urlactivepage.substring(
      ...
      document.write(
        '<script src="' +   home_page + 'feeds/posts/summary?max-results=1&alt=json-in-script&callback=hitungtotaldata"></script>'
      );
    } else {
      jenis = "label";
      ...
      document.write(
        '<script src="' +  home_page +  "feeds/posts/summary/-/" +  lblname1 +  '?alt=json-in-script&callback=hitungtotaldata&max-results=1" ></script>'
      );
    }
  }
}
